...
export const createPlace = asyncHandler(async (req, res) => {
  ...
  if (!title || !description || !links || !img || !Array.isArray(img)) {
    res.status(400).json({ error: 'Title, description, links, and img (array) are required' });
    return;
  }
  ...
  const place = await prisma.places.create({
    data: {
      ...
      infoPlaces: infoPlaces
        ? { create: infoPlaces.map((info) => ({ title: info.title, description: info.description })) }
        : undefined,
    },
  });
  ...
  try {
    ...
    const updatedPlace = await prisma.places.update({
      ...
      data: {
        ...
        region: regionId ? { connect: { id: parseInt(regionId, 10) } } : existingPlace.region,
      },
    });
