...
export const getMultiDayTours = asyncHandler(async (req, res) => {
  ...
  const sortField = sort ? JSON.parse(sort)[0] : 'order'; // Сортируем по order
...
export const createMultiDayTour = asyncHandler(async (req, res) => {
  ...
  const images = img.map((image) =>
    typeof image === 'object' && image.rawFile?.path ? `/uploads/${image.rawFile.path}` : image
  );
...
// @desc    Update order of multiple tours
// @route   PUT /api/multidaytours/order
// @access  Private
export const updateToursOrder = asyncHandler(async (req, res) => {
  const { orderedTours } = req.body;

  try {
    await Promise.all(
      orderedTours.map(async (tour, index) => {
        await prisma.multiDayTours.update({
          where: { id: tour.id },
          data: { order: index },
        });
      })
